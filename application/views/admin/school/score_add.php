    ...
    <tr>
        ...
        <td valign="top" width="100%" height="100%">
            <div style="margin-left: 150px;">
                ...
                <form method="post" action="scoreAddAct">
                    <br /><br />
                    学生省份：*
                    <select name="province_id">
                        <option value="0">请选择省份</option>
                        <?php foreach ($province as $val):?>
                        <option value="<?php echo $val->id;?>" <?php echo ($scoreInfo->province_id == $val->id) ? "selected" : ''?>><?php echo $val->name;?></option>
                        <?php endforeach;?>
                    </select>
                    <br /><br />
                    专业选择：*
                    <select name="professional_id">
                        ...
                        <?php foreach ($professionalArr as $val):?>
                        <option value="<?php echo $val->id;?>" <?php echo ($scoreInfo->professional_id == $val->id) ? "selected" : ''?>><?php echo $val->name;?></option>
                        <?php endforeach;?>
                    </select>
                    <br /><br />
                    年份：*
                    <select name="year">
                        ...
                        <option value="<?php echo $i;?>" <?php echo ($scoreInfo->year == $i) ? "selected" : ''?>><?php echo $i;?></option>
                        <?php endfor;?>
                    </select>
                    <br /><br />
                    考生类别：*
                    <select name="candidates_category">
                        ...
                        <option value="0" <?php echo $scoreInfo->candidates_category == 0 ? "selected" : ''?>>理科</option>
                        <option value="1" <?php echo $scoreInfo->candidates_category == 1 ? "selected" : ''?>>文科</option>
                    </select>
                    <br /><br />
                    录取批次：*
                    <select name="admission_batch">
                        <option value="0" <?php echo $scoreInfo->admission_batch == 0 ? "selected" : ''?>>第一批</option>
                        <option value="1" <?php echo $scoreInfo->admission_batch == 1 ? "selected" : ''?>>第二批</option>
                        <option value="2" <?php echo $scoreInfo->admission_batch == 2 ? "selected" : ''?>>第三批</option>
                        <option value="3" <?php echo $scoreInfo->admission_batch == 3 ? "selected" : ''?>>专科</option>
                        <option value="4" <?php echo $scoreInfo->admission_batch == 4 ? "selected" : ''?>>提前</option>
                    </select>
                    <br /><br />
                    最低分：
                    <input type="text" value="<?php echo !empty($scoreInfo->min_score) ? $scoreInfo->min_score : ''?>" name="min_score">
                    <br /><br />
                    最高分：
                    <input type="text" value="<?php echo !empty($scoreInfo->max_score) ? $scoreInfo->max_score : ''?>" name="max_score">
                    <br /><br />
                    平均分：
                    <input type="text" value="<?php echo !empty($scoreInfo->avg_score) ? $scoreInfo->avg_score : ''?>" name="avg_score">
                    <br /><br /><br />
                    <input type="hidden" value="<?php echo !empty($scoreInfo->id) ? $scoreInfo->id : ''?>" name="id">
